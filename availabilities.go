...
func GetNewAvailabilities(ctx context.Context, log *zap.Logger, now time.Time, groundID string, months int) ([]AvailabilityDetailed, error) {
	...
	// pick random proxy
	randRegion := proxy.Proxies[rand.Intn(len(proxy.Proxies))]
...
func DoAvailabilitySync(ctx context.Context, log *zap.Logger, fs *firestore.Client, ifdb influxdb2.Client, now time.Time, groundID string) error {
	...
	if len(deltas) == 0 {
		log.Debug("no deltas found for site")
		return nil
	}

	err = UpdateNewDeltas2(ctx, log, ifdb, deltas, now)
	if err != nil {
		return err
	}
...
func UpdateNewDeltas2(ctx context.Context, log *zap.Logger, ifdb influxdb2.Client, newDeltas []CampsiteDelta, now time.Time) error {
	...
	for _, delta := range newDeltas {
		...
		newStateFields[delta.NewState] += 1
		...
		availabilityTags := map[string]string{
			"ground": delta.GroundID,
			"site":   delta.SiteID,
		}

		availabilityFields := map[string]interface{}{
			"available": available,
			"state":     delta.NewState,
		}
		writeAPI.WritePoint(write.NewPoint("availability", availabilityTags, availabilityFields, delta.DateAffected))

		deltaTags := map[string]string{
			"ground": delta.GroundID,
			"site":   delta.SiteID,
		}
		...
		deltaFields := map[string]interface{}{
			"available":           available,
			"target_date":         delta.DateAffected,
			"days_to_target_date": math.Floor(delta.DateAffected.Sub(now).Hours() / 24),
			"old_state":           delta.OldState,
			"new_state":           delta.NewState,
		}
		writeAPI.WritePoint(write.NewPoint("delta", deltaTags, deltaFields, now))
...
// 	deltas, avails, err := GetMonthDeltas(ctx, log, fs, now, 3)
// 	if err != nil {
// 		return err
// 	}
